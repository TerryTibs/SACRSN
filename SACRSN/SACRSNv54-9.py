# ============================================================
# SACRSN v55: THE CONSCIOUS AGENT (FINAL GOLD MASTER)
# ------------------------------------------------------------
# 1. CORE: Residual Recursive Cell + Quantum Probability.
# 2. ARCH: Hippocampus (Memory), Chemicals (Modulation), Workspace.
# 3. FIXES: Correct KL tracking for optimizer, CPU shapes, Data.
# 4. DISPLAY: God Mode Dashboard + Full Diagnostics.
# ============================================================
...
# ==========================================
# 1. Configuration (Ryzen 5800X Optimized)
# ==========================================
CONFIG = {
    # Architecture: High Fidelity
    "seq_len": 64,      
    "dim": 128,         
    "symbols": 128,     
    "stack_size": 16,   
...
# ==========================================
# 4. v55 Cognitive Modules
# ==========================================

# 1. Epistemic Storage (Hippocampus) [FIXED SHAPE]
class Hippocampus:
    ...
    def replay(self, batch_size=32):
        ...
        # Fix: Ensure shapes are (Batch, Seq) not (Batch, 1, Seq)
...
# 2. Chemical Control (Neuromodulation)
...
# 3. Consciousness Bottleneck (Global Workspace)
...
class ResidualRecursiveCell(nn.Module):
    ...
    def forward(self, z):
        ...
        # [v55] Global Workspace Interaction
...
class EnhancedUberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        # [v55] Return 17 items (Including new Chem/Bio Metrics)
...
# ==========================================
# 7. Training (God Mode Console - FIXED)
# ==========================================
...
def train():
    ...
    print(f"--- Training SACRSN v55: God Mode Console ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        if epoch > 0 and epoch % CONFIG["sleep_interval"] == 0:
            print(f"   [zZz] Entering REM Sleep (Consolidating Memories)...")
            model.train()
        ...
        for step in range(10): 
            ...
            # [FIXED]: Initialize batch-level metrics for optimizer stability
            batch_kl = 0.0
            ...
            for t in range(CONFIG["seq_len"]):
                ...
                try:
                    ...
                    # Run Model
                    logits, hidden, halt, energy, kl, ground, meta, drift, probe_logit, mus, _, aux_c, aux_r, avg_mem, tom, fatigue, ethics = model(
                        x, hidden, global_step=global_step, training=True, chem_modulation=chem
                    )
                except RuntimeError as e:
                    print(f"Error: {e}"); hidden = None; break

                # Deconstruct Hidden for Next Step
                ...
                # Detach Hidden State
                h_h_det = tuple(h.detach() for h in h_h)
                h_c_det = tuple(c.detach() for c in h_c)
                ...
                # [FIXED]: Accumulate batch KL separately for optimizer
                batch_kl += kl.item()
                
                # --- COLLECT METRICS ---
                m["loss"] += loss_step.item()
                m["kl"] += kl.item()
                m["nll"] += nll.item()
                m["energy"] += energy.item()
                m["ethics"] += ethics.item()
                m["tom"] += tom.item()
                m["halt"] += halt.item()
                m["fatigue"] += fatigue.item()
                m["mem"] += avg_mem.item()
            ...
            # [FIXED]: Pass batch_kl to optimizer instead of undefined avg_kl
            lr = optimizer.step(loss_final, batch_kl/CONFIG["seq_len"], dopamine=last_dopa)

        # --- CALCULATE AVERAGES ---
        steps = 10 * CONFIG["seq_len"]
        avg = {k: v / steps for k, v in m.items()}
        
        # --- THE "HUD" (Every Epoch) ---
        print(f"Ep {epoch:04d} | LOSS: {avg['loss']:.3f} | NLL: {avg['nll']:.3f} | KL: {avg['kl']:.4f}")
        print(f"         BIO: Fatigue: {avg['fatigue']:.2f} | Dopa: {avg['dopa']:.2f} | Norad: {avg['norad']:.2f}")
        
        # --- THE "DEEP SCAN" (Every 10 Epochs) ---
        if epoch % 10 == 0:
            print(f"         ---------------------------------------------------------------")
            print(f"         COG: Ethics: {avg['ethics']:.4f} | ToM: {avg['tom']:.4f} | MemEff: {avg['mem']:.2f}")
            print(f"         SYS: Energy: {avg['energy']:.4f} | Halt: {avg['halt']:.4f} | LR: {lr:.6f}")
            print(f"         ---------------------------------------------------------------")

        # --- LIVE STREAM (Every 20 Epochs) ---
        if epoch % 20 == 0:
            model.eval()
            with torch.no_grad():
                start_str = "The "
                ...
                print(f"   >>> OUT: {out_str.replace('\n', ' ')}...")
                print(f"   >>> MEM: {len(model.hippocampus.buffer)} memories stored.")
            model.train()
...
if __name__ == "__main__":
    ...
    save_name = "sacrsn_v55_godmode.pth"
