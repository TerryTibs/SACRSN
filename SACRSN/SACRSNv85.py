# ============================================================
# SACRSN v85: THE TITANIUM EDITION (Stabilized)
# ------------------------------------------------------------
# 1. FIX: Aggressive clamping in VQ and Softmax.
# 2. FIX: Scaled-down weight initialization (Gain=0.02).
# 3. FIX: Pre-normalization in recursion loops.
# 4. DIAGNOSTICS: Granular NaN detection in training loop.
# ============================================================
...
print(f"Running SACRSN v85 on: {DEVICE}")
...
CONFIG = {
    ...
    "lambda_halt": 0.01,
    "lambda_energy": 0.1,
    "lambda_kl": 0.001,         # Drastically reduced for stability
...
# ==========================================
# 3. Complex Primitives (Stabilized)
# ==========================================
...
class ComplexLinear(nn.Module):
    def __init__(self, dim):
        ...
        # Small initialization to prevent initial explosion
...
class CircadianRhythm(nn.Module):
    ...
    def sleep_cycle(self, model):
        print("\n--- 💤 Entering REM Sleep (Neuroplasticity) ---")
        with torch.no_grad():
...
class HolographicMemory(nn.Module):
    ...
    def forward(self, z, trace_in, ctrl):
        ...
        z_norm = z / (z.abs().max() + 1e-6) # Stability Fix
...
class GroundedMetaGraphVQ(nn.Module):
    def __init__(self, dim, n_symbols):
        super().__init__()
        self.n_symbols = n_symbols
        self.codebook = nn.Parameter(torch.randn(n_symbols, dim*2) * 0.1) # Small init
    ...
    def forward(self, z, prev_symbol_dist=None, prev_entropy=None, temp=1.0, manual_idx=None):
        ...
        # Stability: Pre-normalize z for distance calc
        ...
        d_content = torch.clamp(d_content, min=0.0) # Prevent NaN from negative distance
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            for _ in range(CONFIG["internal_monologue_steps"]):
                z, _, _ = self.cell(z)
                # Normalize inside the loop to prevent explosion
                z_flat = torch.cat([z.real, z.imag], dim=-1)
...
def train():
    ...
    print(f"--- Training SACRSN v85 (Titanium) ---")
...
if __name__ == "__main__":
    FILENAME = "sacrsn_v85_titanium.pth"
