# ============================================================
# SACRSN v31: THE OBSERVABLE EDITION (BATCHED)
# Fixes: PPX Logging Logic, Codebook Collapse
...
CONFIG = {
    ...
    # [TWEAK 1] Lower threshold so it stops overthinking (Steps: 7.7 -> 4.0)
    "act_threshold": 0.95, 
    "ponder_penalty": 0.00001,
    ...
    # Topology & Stability
    # [TWEAK 2] Increase Cost: Force the model to commit to symbols (stops PPX wobble)
    "commitment_cost": 0.25,        
    "graph_bias_scale": 0.8,        
    "symbol_consistency_weight": 0.01,
    "ethical_weight": 0.005,
    
    # [TWEAK 3] Restore Diversity Pressure (Now that it's stable, force it to use more codebook)
    "diversity_weight": 0.5,        
    "entropy_weight": 0.001,
    
    # Training
    "epochs": 3000,
    # [TWEAK 4] Triple the speed. 1e-4 was too safe.
    "learning_rate": 3e-4, 
    "grad_clip": 0.5,
    "eps": 1e-6,
    "warmup_epochs": 0
...
# [CHANGE] Repeat Data
# Required because if we split short text into 32 columns, the columns are too short to learn.
# This ensures we have enough data volume to support the batch size.
if len(TEXT_DATA) < CONFIG["batch_size"] * 300:
...
# [CHANGE] Split Tensors
# 1. data_tensor (1D): Kept as-is for the visualization tools (prevents crashing).
data_tensor = torch.tensor([char_to_ix[c] for c in TEXT_DATA], dtype=torch.long).to(DEVICE)

# 2. train_tensor (2D): Reshaped for the training loop (32 parallel rows).
n_batches = len(data_tensor) // CONFIG["batch_size"]
train_tensor = data_tensor[:n_batches * CONFIG["batch_size"]]
train_tensor = train_tensor.view(CONFIG["batch_size"], -1) # Shape: [32, Seq_Len]

# ==========================================
# 3. Complex Primitives (UNTOUCHED)
# ==========================================
...
def train():
    ...
    print(f"--- Training SACRSN v31 (Observable) | Batch Size: {CONFIG['batch_size']} ---")
    
    # 1. Define steps based on the MATRIX COLUMNS (not raw text length)
    steps_per_epoch = train_tensor.size(1) 
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            entropy_weight = CONFIG["entropy_weight"] * (1 - epoch / CONFIG["epochs"])
            
            for i in range(steps_per_epoch - 1):
                x = train_tensor[:, i].unsqueeze(1)
                y = train_tensor[:, i+1]
                ...
                # --- [CRITICAL FIX 1] Entropy Math ---
                probs = F.softmax(logits, dim=-1)
                # Ensure the MINUS sign is there: - ( ... )
                loss_entropy = -entropy_weight * (-(probs * torch.log(probs + 1e-8)).sum(dim=-1).mean())
                
                # --- [CRITICAL FIX 2] Buffer Update & PPX ---
                curr_onehot = F.one_hot(sym_idx, CONFIG["n_symbols"]).float()
                with torch.no_grad():
                    avg_onehot = curr_onehot.mean(dim=0)
                    # Slow update (0.01) to stabilize batching
                    model.prev_sym_soft.copy_(model.prev_sym_soft * 0.99 + avg_onehot * 0.01)
                ...
                # --- [CRITICAL FIX 3] Normalization ---
                usage_dist = model.prev_sym_soft.detach() + 1e-10
                usage_dist = usage_dist / usage_dist.sum() # <--- Must normalize or PPX is wrong
                
                # Ensure MINUS sign is here too!
                entropy_val = -(usage_dist * torch.log(usage_dist)).sum()
                total_ppx += torch.exp(entropy_val).item()
                
            scheduler.step()
Ep 0000 | Loss: 3.8060 | Steps: 6.93 | Usage(PPX): 67.4 | LR: 0.000100
Ep 0001 | Loss: 3.7769 | Steps: 7.32 | Usage(PPX): 54.4 | LR: 0.000100
Ep 0002 | Loss: 3.8795 | Steps: 7.69 | Usage(PPX): 23.4 | LR: 0.000100
Ep 0003 | Loss: 3.9115 | Steps: 7.77 | Usage(PPX): 22.2 | LR: 0.000100
Ep 0004 | Loss: 4.0507 | Steps: 7.78 | Usage(PPX): 20.8 | LR: 0.000100
Ep 0005 | Loss: 4.1111 | Steps: 7.60 | Usage(PPX): 21.1 | LR: 0.000100
Ep 0006 | Loss: 4.2320 | Steps: 7.59 | Usage(PPX): 17.7 | LR: 0.000100
Ep 0007 | Loss: 4.2689 | Steps: 7.73 | Usage(PPX): 12.9 | LR: 0.000100
Ep 0008 | Loss: 4.1209 | Steps: 7.85 | Usage(PPX): 12.3 | LR: 0.000100
Ep 0009 | Loss: 4.0160 | Steps: 7.72 | Usage(PPX): 39.7 | LR: 0.000100
Ep 0010 | Loss: 4.4008 | Steps: 7.37 | Usage(PPX): 24.3 | LR: 0.000100
Ep 0011 | Loss: 4.5055 | Steps: 7.62 | Usage(PPX): 8.4 | LR: 0.000100
Ep 0012 | Loss: 4.9457 | Steps: 7.21 | Usage(PPX): 5.3 | LR: 0.000100
Ep 0013 | Loss: 4.6607 | Steps: 6.80 | Usage(PPX): 11.6 | LR: 0.000100
Ep 0014 | Loss: 4.2915 | Steps: 6.82 | Usage(PPX): 10.6 | LR: 0.000100
Ep 0015 | Loss: 4.2871 | Steps: 6.67 | Usage(PPX): 12.6 | LR: 0.000100
Ep 0016 | Loss: 3.8791 | Steps: 6.76 | Usage(PPX): 28.4 | LR: 0.000100
Ep 0017 | Loss: 4.0390 | Steps: 7.57 | Usage(PPX): 18.8 | LR: 0.000100
Ep 0018 | Loss: 4.0040 | Steps: 7.16 | Usage(PPX): 14.6 | LR: 0.000100
Ep 0019 | Loss: 4.0284 | Steps: 7.47 | Usage(PPX): 8.2 | LR: 0.000100
Ep 0020 | Loss: 4.0588 | Steps: 7.22 | Usage(PPX): 8.2 | LR: 0.000100
Ep 0021 | Loss: 3.9623 | Steps: 6.68 | Usage(PPX): 15.7 | LR: 0.000100
Ep 0022 | Loss: 4.0265 | Steps: 6.76 | Usage(PPX): 15.3 | LR: 0.000100
Ep 0023 | Loss: 3.8035 | Steps: 6.68 | Usage(PPX): 24.5 | LR: 0.000100
Ep 0024 | Loss: 4.3612 | Steps: 7.02 | Usage(PPX): 8.8 | LR: 0.000100
Ep 0025 | Loss: 4.0984 | Steps: 7.03 | Usage(PPX): 9.7 | LR: 0.000100

            if epoch % 1 == 0:
                # --- [CRITICAL FIX 4] Logging Divisor ---
                # Must divide by steps_per_epoch, NOT len(data_tensor)
                avg_loss = total_loss / steps_per_epoch
                avg_ponder = total_ponder / steps_per_epoch
                avg_ppx = total_ppx / steps_per_epoch
...
def visualize_all(model):
    ...
    with torch.no_grad():
        # [CHANGE] Use 1D 'data_tensor'
        # Visualization tools expect strict sequential processing (Batch=1).
        # Using the batched 'train_tensor' here would cause a crash.
...
def extract_logic_rules(model, data_tensor):
    ...
    with torch.no_grad():
        # [CHANGE] Use 1D 'data_tensor' (passed from main)
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
...
def anomaly_detector(model):
    ...
    # Dual color plotting for readability
...
if __name__ == "__main__":
    ...
    # 1. Train using Batched 2D Tensor
    trained_model = train()
    ...
    # 2. Analysis using 1D Tensor
    # We pass the global 'data_tensor' (which is 1D) to these functions
    # to avoid the dimensions mismatch crash.
