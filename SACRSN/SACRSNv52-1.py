# ============================================================
# SACRSN v52-1: STABLE SINGULARITY (FINAL FIXED)
# ------------------------------------------------------------
...
# 4. MATH: Soft EAFT, Graves ACT, Unified ELBO.
# 5. DIAGNOSTICS (FIXED):
#    - Fixed 'too many values to unpack' errors using *args.
#    - Fixed 'NaN' loss loops by lowering init LR and adding guards.
#    - Fixed 'imag not implemented' errors in Dream Mode.
# ============================================================
...
def train():
    ...
    print(f"--- Training SACRSN v52 (Stable Singularity) ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            # FIX: Ensure scalar loss for BCE
...
def visualize_all(model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            # FIX: Use *args unpacking for safety
            _, hidden, _, energy, kl, _, _, _, _, _, depth, _, _, eff = model(x, hidden)
...
def dream_mode(model):
    ...
    for _ in range(50):
        ...
        # FIX: Complex casting for dream mode
...
def injection_test(model):
    ...
    # FIX: Get index 4 (kl) from tuple return
...
def plot_attention_dynamics(model):
    print("\n--- Generating Attention EEG (State Activation) ---")
    model.eval()
    
    activations = []
    
    def hook_fn(module, input, output):
        # Capture the output state of the cell (z_proc)
        # output is tuple: (z_proc, halt_logit, stack_logits)
        z_proc = output[0].detach().cpu()
        
        # Calculate magnitude of the complex state |z| = sqrt(r^2 + i^2)
        # Shape: [Batch, Dim] -> [Dim] (since batch=1)
        mag = torch.abs(z_proc).squeeze()
        activations.append(mag.numpy())

    # Register hook on the RecursiveCell to see "thoughts" per step
    ...
    if len(activations) > 0:
        # Stack to create 2D Image: [Time(8), Neurons(64)]
        map_data = np.stack(activations)
        
        plt.figure(figsize=(10, 6))
        plt.imshow(map_data, aspect='auto', cmap='magma', interpolation='nearest')
        plt.colorbar(label="Activation Magnitude |z|")
        plt.title("Recursive State Activity (The 'EEG')")
        plt.ylabel("Recursion Step (Time)")
...
def plot_symbol_utilization(model):
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            # Unpack hidden state carefully
            # hidden = (z, h, c, mem, ptr, sym, ent)
            _, hidden, *args = model(x, hidden)
            curr_sym = hidden[5] 
...
if __name__ == "__main__":
    ...
    torch.save(trained_model.state_dict(), "sacrsn_v52_stable.pth")
