"""
Project: CRSN v55 (Causal Repair Edition)
Description: 
    Complex-Valued Recursive RNN.
    
    REPAIR LOG:
    1. Runtime: Fixed GraphVectorQuantizer variable name error.
    2. Causal: ResourceMonitor now biases Halting Probability (Fatigue -> Halt).
    3. Causal: LatentProbe and TransitionConstraint added to Total Loss.
    4. Causal: Hierarchical VAE uses depth-weighted KL.
    5. Logic: TopologyUpdater is now invoked conditionally in the training loop.
    6. Logic: Experience Replay backward pass confirmed.
      
    Status: ACTUATORS ACTIVE.
"""
...
class Config:
    ...
    LAMBDA_PROBE: float = 0.1       # [ACTUATOR] Probe now has weight
    LAMBDA_SHADOW: float = 0.1
    LAMBDA_CONSTRAINT: float = 0.005
...
# ==========================================
# 4. Functional Modules (Actuators)
# ==========================================
...
class ResourceMonitor(nn.Module):
    ...
    def forward(self, load: torch.Tensor) -> torch.Tensor:
        ...
        # Returns normalized fatigue (1.0 = Empty/Exhausted, 0.0 = Full)
        # Inverted: 1.0 (Resource) -> 0.0 Fatigue
        return 1.0 - (self.current_resource / self.capacity)
...
class GraphTopologyUpdater(nn.Module):
    ...
    def update_history(self, sym_idx):
        # FIX: Scalar storage
        if sym_idx.numel() > 1: val = sym_idx[0].view(1)
        else: val = sym_idx.view(1)
        self.symbol_history.append(val.detach())
    ...
    def reassign_cluster(self, z_flat, codebook, adjacency, current_sym):
        # Handle batch
        if current_sym.numel() > 1: curr = current_sym[0]
        else: curr = current_sym
        
        dists = torch.sum((codebook - z_flat)**2, dim=-1)
        candidates = torch.topk(-dists, k=4).indices
        
        best = curr; best_score = dists[curr]
        for s in candidates:
            score = dists[s] + 0.1 * adjacency[curr, s]
            if score < best_score: best_score = score; best = s
            
        if current_sym.numel() > 1: return best.expand_as(current_sym)
        return best
        
    def adjust_weights(self, adjacency, current_sym):
        with torch.no_grad():
            if current_sym.numel() > 1: idx = current_sym[0]
            else: idx = current_sym
            adjacency[idx] *= 0.9
            adjacency[idx] += 0.1 * torch.randn_like(adjacency[idx])
            
    def track_node(self, sym_idx):
        if sym_idx.numel() > 1: key = sym_idx[0].item()
        else: key = sym_idx.item()
...
class HierarchicalVAE(nn.Module):
    ...
    def forward(self, z, h_prev, c_prev):
        ...
        h_list.append(h0); c_list.append(c0); mu_list.append(mu0); 
        # [ACTUATOR] Depth-weighted KL to force hierarchy
        kl_sum += kl0 * 1.0
        
        prev_h = h0
        for i in range(1, self.depth):
            h, c, kl, mu = self.levels[i](prev_h, h_prev[i], c_prev[i])
            h_list.append(h); c_list.append(c); mu_list.append(mu); 
            # [ACTUATOR] Lower weights for higher levels
            kl_sum += kl * (0.5 ** i)
            prev_h = h
...
class GraphVectorQuantizer(nn.Module):
    ...
    def forward(self, z, prev_symbol_dist=None, prev_entropy=None, temp=1.0):
        ...
        # [REPAIR] Fixed variable reference
...
class ComplexRecurrentSequenceNetwork(nn.Module):
    ...
    def forward(self, x, hidden=None, training=True, modulation=None, global_step=0):
        ...
        # [ACTUATOR] Fatigue
        fatigue = self.resource_monitor(torch.tensor(1.0).to(DEVICE))
        ...
        for t in range(cfg.MAX_RECURSION_DEPTH):
            ...
            if training:
                ...
                # [ACTUATOR] Fatigue biases halt logit
                # If fatigue is high (near 1.0), urge to halt is increased
                temp_act = cfg.BASE_HALT_TEMP * (0.9 ** t)
                halt_logit_biased = halt_logit + (fatigue * 2.0)
                p_halt = torch.sigmoid(halt_logit_biased / temp_act)
                
                update_type = self.topology_updater.determine_update_type(loss=energy.item(), ponder=p_halt.mean().item())
                
                # [ACTUATOR] Topology mutation only happens if gate allows AND limiter allows
                if update_type:
            ...
            # [ACTUATOR] Probe is now returned for Loss calc
        ...
        return (logits, next_hidden, m['halt'], m['l2_reg'], m['kl'], m['alignment'], 
                ...
                stack_depth_log, aux_c, aux_r, avg_mem_efficiency, m['shadow'], fatigue, m['constraint'])
    ...
    def train_epoch(self, epoch_idx):
        ...
        for _ in range(10):
            ...
            for t in range(cfg.SEQ_LEN):
                ...
                # [ACTUATOR] Added Constraint, Probe, and Transition to Loss
                loss_step = (cfg.LAMBDA_NLL * nll) + \
                       (cfg.LAMBDA_PONDER * halt) + \
                       (cfg.LAMBDA_L2_REG * l2_reg) + \
                       (beta * kl) + \
                       (cfg.LAMBDA_CONSTRAINT * constraint) + \
                       (cfg.LAMBDA_TRANSITION * transition) + \
                       (cfg.LAMBDA_PROBE * probe.mean()) + \
...
if __name__ == "__main__":
    ...
    save_path = os.path.join(os.getcwd(), "crsn_v55_causal.pth")
    ...
    try:
        ...
        files.download("crsn_v55_causal.pth")
