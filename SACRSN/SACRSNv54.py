# ============================================================
# SACRSN v54: THE FULL-STACK COGNITIVE ARCHITECTURE
# ------------------------------------------------------------
# 1. BIOLOGY (v52): Complex-Valued, EBM Graph, Variational KL.
# 2. PSYCHOLOGY (v33): Belief Reframing, Ethical Gating, Pacing.
# 3. PHYSIOLOGY (v54): Homeostatic Regulation (Energy/Fatigue).
# 4. SOCIAL (v54): Mirror System (Theory of Mind/The Other).
# 5. DIAGNOSTICS: Full Suite + Energy & ToM metrics.
# ============================================================
...
CONFIG = {
    ...
    "lambda_drift": 0.05,
    "lambda_probe": 0.1,
    "lambda_tom": 0.1,          # [NEW] Theory of Mind Loss
    ...
    # Physiology Parameters [NEW]
    "metabolic_capacity": 100.0,
    "metabolic_decay": 0.5,     # Cost per ponder step
    "metabolic_recovery": 0.1,  # Passive recovery per step
...
class HomeostaticRegulator(nn.Module):
    """
    Manages the 'Energy Budget' of the system.
    Forward pass takes 'ponder_cost' (effort) and returns 'fatigue'.
    High Fatigue -> Higher Temp (Noise) + Lower ACT Threshold (Lazy).
    """
...
class EnhancedUberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        # Use simple ACT depth estimate for effort so far (approx 1.0 per step)
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            # Try to predict the thought using the "Other" graph
...
def train():
    ...
    print(f"--- Training SACRSN v54 (Full Cognitive Stack) ---")
...
def visualize_all(model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            # Use *args to catch extra returns
...
def plot_attention_dynamics(model):
    ...
    def hook_fn(module, input, output):
        ...
        q_flat = torch.cat([q.real, q.imag], dim=-1)
        if hasattr(module, 'context_proj'): q_flat = q_flat
        k_flat = torch.cat([k.real, k.imag], dim=-1)
...
def plot_homeostasis(model):
    ...
    # Simulate a run to see energy depletion
    model.eval()
    energy_levels = []
    # Reset regulator for visualization
...
def plot_theory_of_mind(model):
    ...
    # Compare Self-Graph vs Mirror-Graph
...
if __name__ == "__main__":
    ...
    torch.save(trained_model.state_dict(), "sacrsn_v54_fullstack.pth")
