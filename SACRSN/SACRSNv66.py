# ============================================================
# SACRSN v66: THE CIRCADIAN EDITION (LIVING SYSTEM)
# ------------------------------------------------------------
# 1. MATH: Complex RoPE, EBM Graph, Variational KL.
# 2. ARCHITECTURE: Complex Mixture of Experts (MoE) Cell.
# 3. DYNAMICS: Sleep Cycles (Synaptic Pruning + Energy Reset).
# 4. OBJECTIVE: Extrinsic (NLL) + Intrinsic (Curiosity).
# 5. FULL STACK: Physiology, Social, Psychology, Biology.
# ============================================================
...
CONFIG = {
    ...
    "num_experts": 4,           # For Complex MoE
    "rope_base_freq": 10000.0,  # For RoPE
    ...
    "lambda_probe": 0.1,
    "lambda_tom": 0.1,
    "lambda_curiosity": 0.02,   # [NEW] Intrinsic Motivation
    ...
    # Physiology (Sleep & Energy)
    "enable_homeostasis": True, 
    "metabolic_capacity": 100.0,
    "metabolic_decay": 0.5,
    "metabolic_recovery": 0.1,
    "sleep_threshold": 0.2,     # Trigger sleep when energy < 20%
    "pruning_threshold": 0.1,   # Prune synapses with prob < 10%
...
# [UPGRADE 1] Complex Rotary Positional Embeddings
class ComplexRoPE(nn.Module):
    def __init__(self, dim, max_len=1000):
        super().__init__()
        # Frequencies: 10000^(-2i/d)
        freqs = 1.0 / (CONFIG["rope_base_freq"] ** (torch.arange(0, dim, 2).float() / dim))
        t = torch.arange(max_len).float()
        freqs_t = torch.outer(t, freqs) # [MaxLen, Dim/2]
        # In complex domain, rotation is e^(i*theta)
        # We need to broadcast this to [1, Seq, Dim] (complex)
        self.register_buffer("freqs_cis", torch.polar(torch.ones_like(freqs_t), freqs_t))

    def forward(self, z):
        # z: [Batch, 1, Dim] or [Batch, Seq, Dim]
        # Since input is usually 1 token at a time in ACT, we handle single step
        # Ideally this is used at the sequence level. Here we assume z is already embedded.
        # For simplicity in recurrent scalar processing, we apply a fixed phase shift per depth step?
        # No, RoPE is for sequence position. 
        return z 

    def apply_to_sequence(self, z_seq):
        # z_seq: [Batch, Seq, Dim]
        # Expand freqs to match dim
        cis = self.freqs_cis[:z_seq.shape[1]] # [Seq, Dim/2]
        # We need to repeat for real/imag interleaving or just elementwise multiplication in complex
        # Since z is already complex [B, S, D], we assume D matches freq count
        # Map [Seq, D/2] -> [Seq, D] via repetition for complex broadcasting?
        # Actually, if z is dim D complex numbers, we need D rotation angles.
        # But standard RoPE is for pairs. In Complex domain, 1 number = 1 pair.
        # So we need D frequencies.
        cis = torch.cat([cis, cis], dim=-1) # Hack to match dim if needed, but better:
        # Re-calc for full dim
        return z_seq * cis.unsqueeze(0) 

# [UPGRADE 2] Complex Mixture of Experts
class ComplexMoE(nn.Module):
    ...
    def forward(self, z):
        ...
        weights = F.softmax(self.gate(z_flat), dim=-1) # [Batch, Experts]
        
        out = torch.zeros_like(z)
        for i, expert in enumerate(self.experts):
            # Expert output: [Batch, Dim]
            # Weight: [Batch, 1]
...
# ==========================================
# 5. Core Processor (with Complex MoE)
# ==========================================
class ResidualRecursiveCell(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        # [UPGRADE 2] Replaced ComplexLinear with ComplexMoE
...
class EnhancedUberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        halt_cost, energy_cost, kl_cost, ground_cost, meta_cost, drift_cost, probe_logit, tom_cost, curiosity_score = 0, 0, 0, 0, 0, 0, 0, 0, 0
        mem_efficiency_log = []
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            # [UPGRADE 4] Intrinsic Curiosity
            # High energy in Mirror System = Novelty. 
            _, mirror_energy = self.mirror_system(z, current_soft_sym, temp=current_temp)
            tom_cost += mirror_energy
            
            # Novelty Reward: Difference between self-quantized and mirror-projected state
            # If self is confident but mirror is surprised -> Discovery.
        ...
        return logits, next_hidden, halt_cost, energy_cost, kl_cost, ground_cost, meta_cost, drift_cost, probe_logit, (avg_mu_l1, avg_mu_l2), stack_depth_log, aux_class, aux_recon, avg_mem_efficiency, tom_cost, fatigue, curiosity_score

# ==========================================
# 7. Training & Sleep Logic
# ==========================================
# [UPGRADE 3] Sleep & Pruning
def sleep_and_consolidate(model, threshold=0.1):
    print("\n--- 💤 Entering REM Sleep (Synaptic Pruning & Consolidation) ---")
    with torch.no_grad():
        # 1. Prune Weak Connections in Graph
        adj = model.vq.adjacency_energy
        probs = torch.sigmoid(adj)
        mask = probs > threshold
        pruned_count = (~mask).sum().item()
        # Set pruned connections to very negative logits (effectively 0 probability)
        model.vq.adjacency_energy.data[~mask] = -10.0
        
        # 2. Consolidate Experts (Normalization)
        # Normalize weights of experts to prevent explosion
        ...
        # 3. Reset Homeostasis (Full Energy)
        model.homeostasis.energy.fill_(model.homeostasis.capacity)
        
    print(f"--- ⚡ Pruned {pruned_count} weak connections. Energy restored. ---")
...
def train():
    ...
    print(f"--- Training SACRSN v66 (Circadian) ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            try:
                # Returns 17 values now
                logits, hidden, halt, energy, kl, ground, meta, drift, probe_logit, mus, _, aux_c, aux_r, _, tom, fatigue, curiosity = model(x, hidden, global_step=global_step)
            ...
            # Intrinsic Motivation implies slightly modifying loss to seek surprise
            # Reward: Higher curiosity -> Lower Loss
            ...
            loss = CONFIG["lambda_nll"] * nll + \
                   warmup_factor * (
                       ...
                       L_prior
                   ) - (CONFIG["lambda_curiosity"] * curiosity) # Intrinsic Reward
        ...
        # Check Sleep Condition (Low Energy)
        # Using a moving average of fatigue for the epoch
        avg_fatigue = metrics["fatigue"] / count if count > 0 else 1.0
        if avg_fatigue < CONFIG["sleep_threshold"]:
            sleep_and_consolidate(model, threshold=CONFIG["pruning_threshold"])
            hidden = None # Clear working memory after sleep
...
def plot_homeostasis(model):
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            ...
            fatigue = output[-2] # Index shifted due to added return
            hidden = output[1]
...
if __name__ == "__main__":
    FILENAME = "sacrsn_v66_circadian.pth"
    
    # 1. Train
    trained_model = train()
    
    # 2. Final Sleep (Consolidate before save)
    sleep_and_consolidate(trained_model, threshold=0.15)
