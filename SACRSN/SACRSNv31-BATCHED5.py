# ============================================================
# SACRSN v31: THE NARRATIVE ISOLATION EDITION
# Philosophy: "32 Solitary Minds, 1 Optimizer"
# Fixes: Ontological collapse caused by semantic averaging
# ============================================================
...
CONFIG = {
    ...
    # Because minds are now isolated, we restore strong pressure.
...
# Repeat data to ensure "Deep Time" (long columns)
...
class GraphMemoryVQ(nn.Module):
    ...
    def forward(self, z, prev_symbol_idx=None):
        ...
        if prev_symbol_idx is not None:
            # Batch-safe Prior: Each sample looks up its OWN previous symbol row
...
class EthicalConstraint(nn.Module):
    ...
    def forward(self, prev_sym, curr_sym, adjacency):
        ...
        # Calculates loss per-sample, preserving individual agency
...
class UberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, prev_sym=None):
        ...
        for t in range(CONFIG["max_recursion_depth"]):
            ...
            # In batch mode, current_sym is [32] and prev_sym is [32].
            # EthicalConstraint computes vector of 32 losses. This preserves individual guilt.
            ...
            # Ponder cost is technically per-sample here (vector)
...
def train():
    ...
    print(f"--- Training SACRSN v33 (Narrative Isolation) ---")
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(steps_per_epoch - 1):
                ...
                # We do NOT average the probabilities (soft mean) because that destroys meaning.
                # We take the HARD choices (indices) made by the 32 discrete minds, 
                # convert them to 32 one-hots, sum them, and normalize.
                # This preserves "spikiness" (specific choices) rather than "grey goo".
                curr_onehot = F.one_hot(sym_idx, CONFIG["n_symbols"]).float()
                
                with torch.no_grad():
                    # Sum across batch, then normalize. This represents "What the group actually chose"
                    # rather than "What the group sort of thought about".
                    group_choice_dist = curr_onehot.sum(dim=0)
                    group_choice_dist = group_choice_dist / (group_choice_dist.sum() + 1e-9)
                    model.prev_sym_soft.copy_(model.prev_sym_soft * 0.99 + group_choice_dist * 0.01)
                
                # Diversity: Calculated per-sample contribution to the whole
