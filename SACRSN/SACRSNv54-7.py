# ============================================================
# SACRSN v54-7-CPU-MAX: DYNAMIC HIERARCHY EDITION
# ------------------------------------------------------------
# 1. ARCH: Dynamic VAE Depth (Supports 4+ levels of introspection).
# 2. CONFIG: Maximum CPU Capacity (128dim / 64seq / 8act / 4meta).
# 3. FIXES: Generalized hidden state handling for N levels.
# ============================================================
...
# ==========================================
CONFIG = {
    # Architecture: Pushed for Zen 3 Cores
    "seq_len": 64,      # DOUBLED: Reads full sentences (Zen 3 cache handles this easily)
    "dim": 128,         # DOUBLED: "Big Brain" mode. 4x compute cost, but you have the cores.
    "symbols": 128,     # DOUBLED: Matches dim. No bottleneck.
    "stack_size": 16,   # Deeper memory stack
    "max_depth": 6,     # INCREASED: 6 thinking steps per char (Good balance for 6 cores)
    "act_threshold": 0.99,
    
    # Training
    "epochs": 1000,
    "lr": 1e-3,         # Lowered slightly (1e-3) because larger models need stability
    "grad_clip": 1.0,
    "eps": 1e-6,
    "temp": 1.0,
    "residual_weight": 0.1,
    
    # Dynamic Loss (Tuned for larger capacity)
    "lambda_nll": 1.0,
    "lambda_halt": 0.01,
    "lambda_energy": 0.01,
    "lambda_kl": 0.05,
    "lambda_diversity": 0.15, # Increased to force usage of all 128 symbols
    "lambda_ground": 0.1,
    ...
    # Annealing
    "kl_cycle_len": 250, # Slower cycle for larger brain
    "kl_max_beta": 0.05,
    
    # Meta / Psych
    "meta_alpha": 0.1,
    "meta_lr": 0.01,
    "ground_dim": 32,   # Increased ground dimension
    "entropy_delta_thresh": 0.5,
    "meta_hierarchy_depth": 1,
    "reframe_window": 8,
...
# ==========================================
# 4. Advanced Modules (Dynamic Upgrade)
# ==========================================
...
# [UPGRADED] Supports Dynamic Depth
class HierarchicalMetaTracker(nn.Module):
    ...
    def forward(self, z, h_prev_tuple, c_prev_tuple):
        # h_prev_tuple is now a tuple of N tensors
        h_current_list = []
        c_current_list = []
        mu_list = []
        total_kl = 0
        
        # Level 0 (Connects to input z)
        h0_prev, c0_prev = h_prev_tuple[0], c_prev_tuple[0]
        h0, c0, kl0, mu0 = self.levels[0](z, h0_prev, c0_prev)
        h_current_list.append(h0)
        c_current_list.append(c0)
        mu_list.append(mu0)
        total_kl += kl0
        
        # Deeper Levels (Connect to previous level)
        prev_h = h0
        for i in range(1, self.depth):
            h_prev, c_prev = h_prev_tuple[i], c_prev_tuple[i]
            h, c, kl, mu = self.levels[i](prev_h, h_prev, c_prev)
            h_current_list.append(h)
            c_current_list.append(c)
            mu_list.append(mu)
            total_kl += kl
            prev_h = h
            
        # Return tuples to keep signature consistent
        return tuple(h_current_list), tuple(c_current_list), total_kl.mean(), tuple(mu_list)
            
    def update_meta_prior(self, mus_tuple):
        for i, mu in enumerate(mus_tuple):
...
# ==========================================
# 6. Master Model (Dynamic Hidden States)
# ==========================================
class EnhancedUberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        if hidden is None:
            ...
            # Dynamic Hidden Init for N levels
            h_vae_h = []
            h_vae_c = []
            for i in range(CONFIG["meta_hierarchy_depth"]):
                level_dim = self.dim // (2 ** i)
                if level_dim < 8: level_dim = 8
                h_vae_h.append(torch.zeros(batch_size, level_dim).to(z.device))
                h_vae_c.append(torch.zeros(batch_size, level_dim).to(z.device))
            h_vae = tuple(h_vae_h)
            c_vae = tuple(h_vae_c)
        ...
        mem_efficiency_log = []
        # Store mus as list of lists (steps x levels)
        posterior_mus_steps = []
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            # Dynamic VAE forward
            h_vae, c_vae, kl_div, mus_tuple = self.hallucination_vae(z, h_vae, c_vae)
            posterior_mus_steps.append(mus_tuple)
        ...
        # Avg mus for updating prior - process list of tuples
        # Transpose steps/levels to levels/steps
        num_levels = CONFIG["meta_hierarchy_depth"]
        avg_mus_list = []
        for lvl in range(num_levels):
            # stack all steps for this level
            mus_lvl = torch.stack([step_tuple[lvl] for step_tuple in posterior_mus_steps])
            avg_mus_list.append(mus_lvl.mean(0))
        avg_mus_tuple = tuple(avg_mus_list)
        
        avg_mem_efficiency = sum(mem_efficiency_log) / len(mem_efficiency_log)
        
        return logits, next_hidden, halt_cost, energy_cost, kl_cost, ground_cost, meta_cost, drift_cost, probe_logit, avg_mus_tuple, stack_depth_log, aux_class, aux_recon, avg_mem_efficiency, tom_cost, fatigue, ethics_cost
...
def train():
    ...
    print(f"--- Training SACRSN v54-CPU-MAX ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for step in range(10): 
            ...
            for t in range(CONFIG["seq_len"]):
                ...
                # DYNAMIC DETACH for arbitrary depth
