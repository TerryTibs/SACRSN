# ============================================================
# SACRSN v48-1-ULTIMATE: THE GOLDEN MASTER
# ------------------------------------------------------------
# 1. CORE: v48 Stable Complex/Context Primitives.
# 2. PSYCHE: v47 Ethical Gating, Reframing, & Pacing (Restored).
# 3. MEMORY: v48 Enhanced Stack with explicit Complex Casting.
# 4. DIAGNOSTICS: Complete Suite (Topology, Scatter, Coherence, Grounding).
# 5. LOGIC: Hybrid Analysis (Theoretical Probabilities + Dynamic Counts).
# ============================================================
...
CONFIG = {
    ...
    # Psychology & Ethics (Restored from v47)
...
# ==========================================
# 3. Complex Primitives (v48 Stable Version)
# ==========================================
...
class ContextAwareAttention(nn.Module):
    def __init__(self, dim):
        ...
        # v48 Fix: Project from dim*2 to dim*2 (match concatenated query)
...
class ComplexAttention(nn.Module):
    ...
    def forward(self, z):
        ...
        attn_scores = torch.matmul(q_flat, k_flat.transpose(-2, -1)) * self.scale
        attn_weights = F.softmax(attn_scores, dim=-1)
        return torch.complex(torch.matmul(attn_weights, v.real), torch.matmul(attn_weights, v.imag))

# ==========================================
# 4. Psychology & Meta-Cognition (Restored v47)
# ==========================================
...
class MetaIntrospectionTracker(nn.Module):
    ...
    def forward(self, z, h_prev, c_prev):
        # v48 Fix: Handle complex inputs gracefully
        ...
        h_prior_mixed = CONFIG["meta_alpha"] * h_step_prior + (1 - CONFIG["meta_alpha"]) * self.meta_h_prior
...
class HierarchicalMetaTracker(nn.Module):
    ...
    def forward(self, z, h_prev_tuple, c_prev_tuple):
        h1_prev, h2_prev = h_prev_tuple; c1_prev, c2_prev = c_prev_tuple
        h1, c1, kl1, mu1 = self.levels[0](z, h1_prev, c1_prev)
        h2, c2, kl2, mu2 = self.levels[1](h1, h2_prev, c2_prev)
        return (h1, h2), (c1, c2), kl1 + kl2, (mu1, mu2)
...
class GroundedMetaGraphVQ(nn.Module):
    ...
    def forward(self, z, prev_symbol_dist=None, prev_entropy=None, temp=1.0):
        ...
        if prev_symbol_dist is not None:
            ...
            d_total = d_content - CONFIG["lambda_energy"] * torch.sigmoid(graph_bias)
        else:
            d_total = d_content
...
class EnhancedMemoryStack(nn.Module):
    ...
    def forward(self, z, mem, ptr, ctrl):
        push, pop, stay = torch.sigmoid(ctrl[:, 0:1]), torch.sigmoid(ctrl[:, 1:2]), torch.sigmoid(ctrl[:, 2:3])
...
# ==========================================
# 7. Master Model (Merged)
# ==========================================
class SacrsnV48Ultimate(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
                image_input=None, audio_input=None, prev_sym_idx=None, global_step=0, training=True):
        ...
        if hidden is None:
            ...
            h_vae = (torch.zeros(batch_size, self.dim).to(z.device), torch.zeros(batch_size, self.dim//2).to(z.device))
            c_vae = (torch.zeros(batch_size, self.dim).to(z.device), torch.zeros(batch_size, self.dim//2).to(z.device))
            ...
            prev_soft_sym = None; prev_entropy = None
        else:
            z_prev, h_vae, c_vae, stack_mem, stack_ptr, prev_soft_sym, prev_entropy = hidden
            z = 0.5 * z + 0.5 * z_prev

        # Unified Metrics Dict (v47 Style with v48 additions)
        metrics = {"halt": 0, "energy": 0, "kl": 0, "ground": 0, "meta_E": 0, "ethics": 0}
        mem_efficiency_log, posterior_mus_l1, posterior_mus_l2, stack_history = [], [], [], []
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            posterior_mus_l1.append(mus[0]); posterior_mus_l2.append(mus[1])
            
            # 3. VQ (v48 Grounded)
            z_q, soft_sym, sym_idx, energy, ent, ground, meta = self.vq(
                z, current_soft_sym, prev_entropy=current_entropy, temp=CONFIG["temp"]
            )
            
            # 4. Psych Reframing (Restored v47 Logic)
            ...
            if training:
                ...
                if reframe_type:
                    meta_stat = self.meta_tracker.stats[sym_idx.item()]
                ...
                if reframe_type == "meaning":
                    ...
                    sym_idx = self.reframer.meaning_reframe(
                        z_flat, self.vq.codebook.detach(), self.vq.adjacency_energy.detach(), sym_idx
                    )
                elif reframe_type == "context":
                    self.reframer.context_reframe(self.vq.adjacency_energy.data, sym_idx)
                
                changed = (sym_idx != current_hard_sym) if current_hard_sym is not None else False
                self.meta_tracker.update(sym_idx, energy.item(), p_halt.mean().item(), changed)
                self.reframer.label_belief(sym_idx)

            # 5. Ethical Loss
            eth_loss = self.ethics(current_hard_sym, sym_idx, self.vq.adjacency_energy)
            current_hard_sym = sym_idx
            ...
            metrics["kl"] += kl_div
            metrics["ground"] += ground
            metrics["meta_E"] += meta
            metrics["ethics"] += eth_loss
        ...
        next_hidden = (z_weighted, h_vae, c_vae, stack_mem, stack_ptr, current_soft_sym, current_entropy)
        mus_avg = (torch.stack(posterior_mus_l1).mean(0), torch.stack(posterior_mus_l2).mean(0))
        avg_stack = torch.stack(stack_history).mean() if stack_history else torch.tensor(0.0)
        avg_mem_eff = sum(mem_efficiency_log) / len(mem_efficiency_log)
        
        return logits, next_hidden, current_hard_sym, metrics, avg_stack, mus_avg, aux_class, aux_recon, avg_mem_eff

# ==========================================
# 8. Training (v48 Adaptive + v47 Hooks)
# ==========================================
class AdaptiveOptimizer:
    ...
    def step(self, loss, kl_value):
        ...
        if len(self.kl_history) > 5:
            volatility = np.std(self.kl_history[-5:])
...
def train():
    model = SacrsnV48Ultimate(vocab_size, CONFIG["dim"]).to(DEVICE)
    optimizer = AdaptiveOptimizer(model, base_lr=CONFIG["lr"])
    print(f"--- Training SACRSN v48-ULTIMATE ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            try:
                logits, hidden, sym_idx, metrics, _, mus, aux_c, aux_r, _ = model(
                    x, hidden, prev_sym_idx=prev_sym, global_step=global_step, training=True
                )
            except RuntimeError as e:
                print(f"RuntimeError at step {i}: {e}. Skipping.")
                hidden = None
                continue
            
            h_z, h_vae, c_vae, h_mem, h_ptr, h_sym, h_ent = hidden
            h_vae = (h_vae[0].detach(), h_vae[1].detach())
            c_vae = (c_vae[0].detach(), c_vae[1].detach())
            hidden = (h_z.detach(), h_vae, c_vae, h_mem.detach(), h_ptr.detach(), 
            ...
            loss_nll = (F.cross_entropy(logits, y, reduction='none') * weight).mean()
            
            buffer = model.prev_sym_soft + 1e-9
            loss_diversity = -CONFIG["lambda_diversity"] * (buffer * torch.log(buffer)).sum()
            
            # v47 Consistency Loss
            ...
            loss = CONFIG["lambda_nll"] * loss_nll + \
                   CONFIG["lambda_halt"] * metrics["halt"] + \
                   CONFIG["lambda_energy"] * metrics["energy"] + \
                   CONFIG["lambda_kl"] * metrics["kl"] + \
                   CONFIG["lambda_ground"] * metrics["ground"] + \
                   CONFIG["lambda_meta_energy"] * metrics["meta_E"] + \
                   CONFIG["ethical_weight"] * metrics["ethics"] + \
                   CONFIG["lambda_aux_class"] * L_aux_c + \
                   CONFIG["lambda_aux_recon"] * L_aux_r + \
                   loss_diversity + loss_consistency
            
            if torch.isnan(loss) or torch.isinf(loss):
                hidden = None; continue
                
            lr = optimizer.step(loss, metrics["kl"].item())
            total_loss += loss.item()
            avg_kl += metrics["kl"].item()

        if epoch % 50 == 0:
            print(f"Ep {epoch:04d} | Loss: {total_loss:.4f} | KL: {avg_kl/len(data_tensor):.4f} | LR: {lr:.6f}")
            if total_loss < 0.1: break
            
    return model

# ==========================================
# 9. Unified Visualization Suite (The Complete Suite)
# ==========================================
def visualize_all(model):
    print("\n--- Generating Unified Diagnostics (Complete Suite) ---")
    model.eval()
    
    symbol_to_char = defaultdict(list)
    hidden, prev_sym = None, None
    stack_history, kl_vals, energy_vals, mem_eff = [], [], [], []
    ...
    for _ in range(200):
        with torch.no_grad():
            logits, hidden, sym_idx, metrics, avg_stack, _, _, _, eff = model(
                x, hidden, prev_sym_idx=prev_sym, training=False
            )
            
            stack_history.append(avg_stack.item() if isinstance(avg_stack, torch.Tensor) else 0)
            kl_vals.append(metrics["kl"].item())
            energy_vals.append(metrics["energy"].item())
    ...
    # Plot 2: Cognitive MRI (Merged)
    fig, (ax1, ax2, ax3, ax4) = plt.subplots(4, 1, figsize=(10, 12), sharex=True)
    ax1.plot(stack_history, color='purple', label='Stack Depth')
    ax1.legend()
    ax2.plot(kl_vals, color='green', label='Introspection (KL)')
    ax2.legend()
    ax3.plot(energy_vals, color='orange', label='Energy')
    ax3.legend()
    ax4.plot(mem_eff, color='cyan', label='Mem Efficiency')
    ax4.legend()
    plt.savefig("2_cognitive_mri.png")
    plt.close()
    
    # Plot 3: Grounding (v48)
    grounding = model.vq.symbol_ground.detach().cpu().numpy()
    plt.figure(figsize=(6,6))
    plt.imshow(grounding[:20], cmap='inferno')
    plt.title("3_symbol_grounding")
    plt.savefig("3_symbol_grounding.png")
    plt.close()

    # Plot 4: Phase Scatter (Restored from v47)
    ...
    plt.title("5_phase_coherence")
    plt.savefig("5_phase_coherence.png")
    plt.close()

    print("Diagnostics saved (5 Charts).")

# [Fixed: v48 Logic for Dream Mode]
...
def anomaly_detector(model):
    ...
    with torch.no_grad():
        for i in range(len(input_tensor) - 1):
            ...
            _, hidden, prev_sym, metrics, _, _, _, _, _ = model(x, hidden, prev_sym_idx=prev_sym, training=False)
            anomalies.append(metrics["ethics"].item()) 

    plt.figure(figsize=(10, 4))
    plt.plot(list(corrupt_text)[1:], anomalies, color='crimson', marker='o')
    plt.title("6_anomaly_detection")
    plt.savefig("6_anomaly_detection.png")
    plt.close()

# [Fixed: v48 Calibrated Injection]
def injection_test(model):
    print("\n--- Introspection Injection (Calibrated) ---")
    model.eval()
    x = torch.tensor([[char_to_ix["T"]]], device=DEVICE)
    _, _, _, metrics_base, _, _, _, _, _ = model(x, None)
    
    # Reduced magnitude (2.0) to prevent NaN crash
    inject = torch.complex(torch.randn(1, 64).to(DEVICE)*2.0, torch.randn(1, 64).to(DEVICE)*2.0)
    
    try:
        _, _, _, metrics_inj, _, _, _, _, _ = model(x, None, injected_thought=inject)
        print(f"Base KL: {metrics_base['kl'].item():.4f} | Injected KL: {metrics_inj['kl'].item():.4f}")
    except RuntimeError:
        print("Injection caused System Shock (Expected behavior for high entropy thought).")

# [Fixed: Hybrid Logic Rules (v48 Prob + v47 Counts)]
def extract_logic_rules_hybrid(model, data_tensor=None):
    print("\n--- Logic Rules (Hybrid Analysis) ---")
    
    # 1. Theoretical Probabilities (v48)
    print("\n[Theoretical] Energy-Based Probabilities:")
    ...
    for i in range(CONFIG["symbols"]):
        ...
        if prob > 0.1: 
            ...
            count += 1
    print(f"Significant Theoretical Paths: {count}")

    # 2. Dynamic Observation (v47 Restored)
    if data_tensor is not None:
        print("\n[Dynamic] Actual Observed Transitions:")
        ...
        with torch.no_grad():
            for i in range(len(data_tensor) - 1):
                x = data_tensor[i].view(1, 1)
                # Pass training=False to avoid messing up history stats
                _, hidden, sym_idx, _, _, _, _, _, _ = model(
                    x, hidden, prev_sym_idx=prev_sym, training=False
                )
                if prev_sym is not None:
        ...
        print(f"{'FROM':<6} | {'TO':<6} | {'OBSERVED COUNT'}")
        print("-" * 35)
...
def create_dashboard(model):
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            _, hidden, _, _, _, _, _, _, _, = model(x, hidden)
            z = hidden[0].cpu().squeeze()
            if z.dim() > 0:
                reals.append(z.real[0].item())
                imags.append(z.imag[0].item())
            else:
                reals.append(z.real.item())
                imags.append(z.imag.item())
...
if __name__ == "__main__":
    ...
    print("\n--- Saving Model ---")
    torch.save(trained_model.state_dict(), "sacrsn_v48_ultimate.pth")
    
    visualize_all(trained_model)
    extract_logic_rules_hybrid(trained_model, data_tensor)
