# ============================================================
# SACRSN v54-8: THE CONSCIOUS AGENT (CPU OPTIMIZED)
# ------------------------------------------------------------
# 1. ARCHITECTURE: +Hippocampus, +Neuromodulation, +Global Workspace.
# 2. DATA: Offline "Pocket Library" (Philosophical Text).
# 3. TRAINING: Sleep/Replay Cycles + Chemical Learning Rate.
# 4. DIAGNOSTICS: Complete Suite (v54 Verified).
# ============================================================
...
# ==========================================
# 1. Configuration (v55 CPU High-Fidelity)
# ==========================================
CONFIG = {
    # Architecture: The "Genius" Setup
    "seq_len": 64,      
    "dim": 128,         
    "symbols": 128,     
    "stack_size": 16,   
    ...
    # v55 New Components
    "workspace_dim": 32,      # The "Consciousness Bottleneck" size
    "hippocampus_size": 2000, # Size of episodic memory
    "sleep_interval": 50,     # Train on memories every N epochs
    "sleep_steps": 20,        # How many replay steps per sleep cycle
...
# ==========================================
# 4. v55 NEW Cognitive Modules
# ==========================================

# [NEW] Epistemic Storage
class Hippocampus:
    def __init__(self, capacity=2000):
        self.buffer = deque(maxlen=capacity)
        
    def memorize(self, x, y, loss):
        # Only memorize "surprising" events (High Loss)
        # Using a threshold relative to recent performance would be better,
        # but > 2.5 is a solid heuristic for early training.
        if loss > 2.5:
    ...
    def replay(self, batch_size=32):
        ...
        x_stack = torch.stack([item[0] for item in batch]).to(DEVICE)
        y_stack = torch.stack([item[1] for item in batch]).to(DEVICE)
...
class Neuromodulator(nn.Module):
    ...
    def forward(self, current_loss, current_entropy):
        ...
        # If current loss < average, we are doing better than expected -> Release Dopamine
        # Dopamine boosts Learning Rate (Neuroplasticity)
        prediction_error = self.avg_loss - current_loss.detach()
        dopamine = torch.clamp(prediction_error, 0.0, 1.0)
        
        # NORADRENALINE: Uncertainty / Surprise
        # If entropy is high, we are confused -> Release Noradrenaline
        # Noradrenaline lowers Temperature (Sharpen Focus)
        noradrenaline = torch.sigmoid((current_entropy.detach() - self.avg_entropy) * 2.0)
        
        return dopamine, noradrenaline

# [NEW] Consciousness Bottleneck
class GlobalWorkspace(nn.Module):
    def __init__(self, dim, workspace_dim=32):
        super().__init__()
        self.dim = dim
        self.workspace_dim = workspace_dim
        # Compress to bottleneck
        self.write = nn.Linear(dim * 2, workspace_dim)
        # Broadcast back to brain
        self.broadcast = nn.Linear(workspace_dim, dim * 2)
        self.norm = nn.LayerNorm(dim * 2)
        
    def forward(self, z):
        # Input: Complex State z
        flat = torch.cat([z.real, z.imag], dim=-1)
        
        # 1. Compete for Workspace Access (Bottleneck)
        workspace_state = torch.tanh(self.write(flat))
        
        # 2. Broadcast Global Signal
        broadcast_signal = self.broadcast(workspace_state)
        
        # 3. Integrate (Residual)
        combined = self.norm(flat + broadcast_signal)
        
        # Return as Complex
        z_out = torch.complex(combined[..., :self.dim], combined[..., self.dim:])
        return z_out

# ==========================================
# 5. Existing Advanced Modules
# ==========================================
...
# ==========================================
# 5. Core Processor (with Global Workspace)
# ==========================================
class ResidualRecursiveCell(nn.Module):
    def __init__(self, dim):
        ...
        # [v55] Global Workspace Bottleneck
        self.global_workspace = GlobalWorkspace(dim, CONFIG["workspace_dim"])
    ...
    def forward(self, z):
        ...
        # [v55] Replace simple residual with Global Workspace interaction
        z_proc = self.global_workspace(z_proc)
        
        # Standard Residual add
        z_proc = z_proc + CONFIG["residual_weight"] * z_in
...
# ==========================================
# 6. Master Model (Integrated v55 Modules)
# ==========================================
class EnhancedUberCRSN(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        # [v55] New Modules
        self.hippocampus = Hippocampus(capacity=CONFIG["hippocampus_size"])
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        # [v55] Apply Noradrenaline (Sharpness) from modulation if provided
        # High Noradrenaline -> Lower Temp (Focus)
        if chem_modulation:
            dopa, norad = chem_modulation
            temp_modifier = 1.0 / (1.0 + norad) 
        else:
            temp_modifier = 1.0
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            current_temp = CONFIG["temp"] * temp_modifier
...
# ==========================================
# 7. Training (Modulated & Sleep-Enabled)
# ==========================================
class AdaptiveOptimizer:
    ...
    def step(self, loss, kl_value, dopamine=0.0):
        if math.isnan(kl_value) or math.isinf(kl_value): kl_value = 100.0
        self.kl_history.append(kl_value)
        
        # [v55] Apply Dopamine Boost
        # Dopamine (0.0 to 1.0) multiplies base LR
        # We clamp it so it doesn't explode (max 1.5x)
        boost = 1.0 + (dopamine * 0.5) 
...
def train():
    ...
    print(f"--- Training SACRSN v55: The Conscious Agent ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        # [v55] Sleep / Replay Cycle
        if epoch > 0 and epoch % CONFIG["sleep_interval"] == 0:
            print(f"   [zZz] Entering REM Sleep (Replaying {CONFIG['sleep_steps']} Memories)...")
            model.train()
            for _ in range(CONFIG["sleep_steps"]):
                rx, ry = model.hippocampus.replay()
                if rx is not None:
                    # Quick training step on memory
                    # Just standard loss, no chemicals
                    r_logits, r_hidden, *r_rest = model(rx, None, training=True)
                    r_loss = F.cross_entropy(r_logits, ry.view(-1))
                    optimizer.opt.zero_grad()
                    r_loss.backward()
                    optimizer.opt.step()
        
        # Standard Waking Loop
        for step in range(10): 
            ...
            for t in range(CONFIG["seq_len"]):
                ...
                try:
                    # Pass previous chemical state to modulate this step
                    chem = (last_dopa, last_norad)
                    logits, hidden, halt, energy, kl, ground, meta, drift, probe_logit, mus, _, aux_c, aux_r, _, tom, fatigue, ethics = model(
                        x, hidden, global_step=global_step, training=True, chem_modulation=chem
                    )
                ...
                # [v55] Memorize surprise
                model.hippocampus.memorize(x, y, loss_step.item())
                
                # [v55] Update Chemicals
                dopa, norad = model.neuromodulator(loss_step, entropy)
                last_dopa = dopa.item()
                last_norad = norad.item()

            loss_final = loss_seq / CONFIG["seq_len"]
            # Pass Dopamine to Optimizer
            lr = optimizer.step(loss_final, avg_kl/CONFIG["seq_len"], dopamine=last_dopa)
            total_loss += loss_final.item()

        print(f"Ep {epoch:04d} | Loss: {total_loss/10:.4f} | KL: {avg_kl/(10*CONFIG['seq_len']):.4f} | Dopa: {last_dopa:.2f} | Norad: {last_norad:.2f}")
...
def injection_test(model):
    ...
    kl_base = out_base[4]
    inject = torch.complex(torch.randn(1, 128).to(DEVICE)*2.0, torch.randn(1, 128).to(DEVICE)*2.0)
    try:
...
if __name__ == "__main__":
    ...
    save_name = "sacrsn_v55_conscious.pth"
