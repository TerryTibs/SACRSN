...
CONFIG = {
    ...
    # Topology & Stability
    "commitment_cost": 0.01,        # Very low to encourage exploration
    "graph_bias_scale": 0.8,        # High to force topological learning
    "symbol_consistency_weight": 0.01,
    "ethical_weight": 0.005,
    "diversity_weight": 0.5,        # Force codebook usage
...
# [CHANGE 2] Data Prep
# Repeat data to ensure we have enough text to create 32 parallel streams
# Original text is too short to be split into 32 columns without repeating
...
# We split into two tensors: 
# 1. 'data_tensor' (1D) for the visual tools (prevents crashing)
# 2. 'train_tensor' (2D) for the training loop
...
def train():
    ...
    print(f"--- Training SACRSN v31 (Observable) ---")
    
    # [CHANGE 3] Iterate Columns
    # Loop over the columns of the batch (Seq Length), not the total data length
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(steps_per_epoch - 1):
                # [CHANGE] Batch Slicing
                # Select the i-th column across all 32 rows
                x = train_tensor[:, i].unsqueeze(1) # Shape: [32, 1]
                y = train_tensor[:, i+1]            # Shape: [32]
                ...
                # [CHANGE 4] Entropy Normalization
                # .sum() without .mean() would make the loss 32x larger than intended
                ...
                # [CHANGE 5] Buffer Update
                # Must average over the batch before adding to buffer
                curr_onehot = F.one_hot(sym_idx, CONFIG["n_symbols"]).float()
                with torch.no_grad():
                    # Average the 32 one-hots into 1 probability distribution
                    avg_onehot = curr_onehot.mean(dim=0)
                    model.prev_sym_soft.copy_(model.prev_sym_soft * 0.9 + avg_onehot * 0.1)
                
                # Diversity Loss based on buffer history
                ...
                # [FIX] Calculate True PPX from buffer
...
def visualize_all(model):
    ...
    with torch.no_grad():
        ...
        # Visualization tools expect strict sequential (Batch=1) processing.
        # Passing 'train_tensor' here would cause a dimension mismatch crash.
...
def extract_logic_rules(model, data_tensor):
    ...
    with torch.no_grad():
        # [CHANGE] Use 1D 'data_tensor' (argument passed from main)
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
...
if __name__ == "__main__":
    ...
    # [CHANGE] Visualization Tools
    # Note: 'visualize_all' uses the global 'data_tensor' (1D) internally, 
    # so it does not need arguments changed here, it just needs the tensor to exist.
    visualize_all(trained_model)
    
    # 'extract_logic_rules' takes a dataset argument. We must pass the 1D 'data_tensor'.
