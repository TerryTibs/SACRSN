# ============================================================
# SACRSN v49: THE MATHEMATICALLY REFINED SYSTEM
# ------------------------------------------------------------
# 1. CORE: Complex-Valued, Residual Cell, Context-Attention.
# 2. MATH: EBM Graph, Variational KL, Soft EAFT.
# 3. META: Hierarchical Introspection, Grounding.
# 4. UPGRADES (v49):
#    - Wirtinger-Stable Primitives (Improved ModReLU)
#    - Unpredictability Perception (Dynamic Lambda_E)
#    - Amortized Hallucination Probe (Discriminative Surprise)
#    - Boltzmann ACT (Temperature Scaled Halting)
#    - Semantic Drift Penalty (Trajectory Grounding)
# ============================================================
...
CONFIG = {
    ...
    "lambda_nll": 1.0,
    "lambda_halt": 0.01,
    "lambda_energy": 0.1,       # Base energy weight (Scaled by UP)
    "lambda_kl": 0.1,
    ...
    "lambda_drift": 0.05,       # [NEW] Semantic Drift Penalty
    "lambda_probe": 0.1,        # [NEW] Amortized Probe Loss
    ...
    "act_temp_base": 1.0,       # [NEW] Base Temperature for ACT
...
# ==========================================
# 3. Complex Primitives (Wirtinger Optimized)
# ==========================================
class ComplexLayerNorm(nn.Module):
    ...
    def forward(self, z):
        # eps added inside sqrt for numerical stability
        mag = torch.abs(z)
...
class ModReLU(nn.Module):
    """ 
    [UPGRADE] Wirtinger-Safe ModReLU 
    Ensures gradients flow correctly through magnitude calculation.
    """
    def __init__(self, dim):
        super().__init__()
    ...
    def forward(self, z):
        # Calculate magnitude with stability eps
        mag = torch.abs(z) + CONFIG["eps"] 
        # ReLU applies to (magnitude + bias)
        scale = F.relu(mag + self.bias) / mag
        # z * scale preserves phase, modifies magnitude
        return z * scale
...
# ==========================================
# 4. Advanced Modules & v49 Upgrades
# ==========================================

# [UPGRADE 2] Unpredictability Perception (UP)
class ComplexityModulator(nn.Module):
    """
    Adaptive Energy Scaling.
    High Variance (Complexity) -> High Energy Weight (Focus on structure).
    Low Variance (Simplicity) -> Low Energy Weight (Relax).
    """
    ...
    def forward(self, z):
        # Calculate local variance of the complex magnitude
        mag = torch.abs(z)
        batch_var = mag.var()
        ...
        # Modulate lambda: If current var > expected, increase weight
        # We use a softplus to ensure positive scaling
        modulation = F.softplus(batch_var / (self.running_var + CONFIG["eps"]))
        return modulation

# [UPGRADE 3] Amortized Hallucination Probe
class AmortizedHallucinationProbe(nn.Module):
    """
    Discriminative Introspection.
    A simple probe trained to predict if the model is in a 'Conflicted' state.
    """
    def __init__(self, dim):
        super().__init__()
    ...
    def forward(self, z):
        ...
        # Returns logit for [Hallucination vs Reality]
...
class MetaIntrospectionTracker(nn.Module):
    ...
    def forward(self, z, h_prev, c_prev):
        ...
        kl_div = 0.5 * (
            (mu - h_prior_mixed).pow(2) / prior_var
...
class GroundedMetaGraphVQ(nn.Module):
    def __init__(self, dim, n_symbols):
        ...
        # [UPGRADE 2] Complexity Modulator
    ...
    def forward(self, z, prev_symbol_dist=None, prev_entropy=None, temp=1.0):
        ...
        if prev_symbol_dist is not None:
            ...
            # Scale energy weight by complexity (Unpredictability Perception)
...
# ==========================================
# 5. Core Processor (Residual + Boltzmann ACT)
# ==========================================
...
class EnhancedUberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        # [UPGRADE 5] Capture Initial Grounding State
        z_initial = z.clone()
        ...
        halt_cost, energy_cost, kl_cost, ground_cost, meta_cost, drift_cost, probe_logit = 0, 0, 0, 0, 0, 0, 0
        mem_efficiency_log = []
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            # [UPGRADE 3] Probe (Discriminative Surprise)
            probe_out = self.hallucination_probe(z)
            probe_logit += probe_out
            
            # Memory
            z_q, soft_sym, _, energy, ent, ground, meta = self.vq(
            ...
            # [UPGRADE 4] Boltzmann ACT
            # Scale logit by temperature (decays with depth to force decision)
            temp = CONFIG["act_temp_base"] * (0.9 ** t)
            p_halt = torch.sigmoid(halt_logit / temp)
            ...
            # [UPGRADE 5] Semantic Drift
            # Check deviation from initial thought (Real part distance)
        ...
        # Return all costs for unification
        return logits, next_hidden, halt_cost, energy_cost, kl_cost, ground_cost, meta_cost, drift_cost, probe_logit, (avg_mu_l1, avg_mu_l2), stack_depth_log, aux_class, aux_recon, avg_mem_efficiency
...
class AdaptiveOptimizer:
    ...
    def step(self, loss, kl_value):
        if math.isnan(kl_value) or math.isinf(kl_value):
            kl_value = 100.0
...
def train():
    ...
    print(f"--- Training SACRSN v49 (Mathematically Refined) ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            try:
                logits, hidden, halt, energy, kl, ground, meta, drift, probe_logit, mus, _, aux_c, aux_r, _ = model(x, hidden)
            except RuntimeError as e:
                print(f"Error at step {i}: {e}")
                hidden = None
                continue
            
            h_z, h_h, h_c, h_mem, h_ptr, h_sym, h_ent = hidden
            ...
            # [UPGRADE 3] Probe Loss (Self-Supervised)
            # If Entropy is High (Uncertain), we expect Hallucination (1.0).
            # If Entropy is Low (Certain), we expect Reality (0.0).
            probe_target = torch.sigmoid(entropy).detach()
            L_probe = F.binary_cross_entropy_with_logits(probe_logit.mean(), probe_target)
...
def visualize_all(model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            logits, hidden, _, energy, kl, _, _, _, _, _, depth, _, _, eff = model(x, hidden)
...
def advanced_phase_analysis(model):
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            _, hidden, _, _, _, _, _, _, _, _, _, _, _, _ = model(x, hidden)
            zs.append(hidden[0])
...
def injection_test(model):
    ...
    _, _, _, _, kl_base, _, _, _, _, _, _, _, _, _ = model(x, None)
    inject = torch.complex(torch.randn(1, 64).to(DEVICE)*2.0, torch.randn(1, 64).to(DEVICE)*2.0)
    
    try:
        _, _, _, _, kl_inj, _, _, _, _, _, _, _, _, _ = model(x, None, injected_thought=inject)
    except RuntimeError:
...
def extract_logic_rules(model):
    ...
    for i in range(CONFIG["symbols"]):
        ...
        if prob > 0.1:
            ...
            count += 1
    print(f"Total Significant Transitions: {count}")
...
if __name__ == "__main__":
    ...
    torch.save(trained_model.state_dict(), "sacrsn_v49_refined.pth")
    print("Complete.")
