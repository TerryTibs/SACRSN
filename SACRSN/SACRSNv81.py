# ============================================================
# SACRSN v81: THE STABILIZED MONOLITH (Crash-Proof)
# ------------------------------------------------------------
# 1. FIX: Handled ZeroDivisionError in training loop.
# 2. FIX: Enabled Complex RoPE rotation logic.
# 3. STABILITY: Clamped losses and adjusted LR.
# 4. FEATURES: All previous v80 features included.
# ============================================================
...
print(f"Running SACRSN v81 on: {DEVICE}")
...
CONFIG = {
    ...
    # --- TRAINING ---
    "epochs": 300,
    "lr": 3e-4, # [FIX] Lowered LR for stability
    "grad_clip": 0.5,
    "eps": 1e-6,
    "temp": 1.0,
    "residual_weight": 0.1,
...
class ComplexRoPE(nn.Module):
    ...
    def forward(self, z, step=0):
        # z: [Batch, Dim] (Complex)
        # step: current global step or position
        
        # Ensure we don't go out of bounds
        safe_step = step % self.freqs_cis.shape[0]
        cis = self.freqs_cis[safe_step] # [Dim/2]
        
        # We need to apply this to the complex vector z
        # z is [Batch, Dim], meaning it has 'dim' complex numbers.
        # RoPE usually operates on pairs. Here, our 'dim' is complex numbers.
        # We broadcast the rotation to match.
        
        # Expand cis to match z if necessary, or assuming freq calc matches dim
        # Here we repeat to cover full dim if freqs_cis is smaller
        if cis.shape[0] * 2 == self.dim:
             # This implies 1 complex number per pair of reals?
             # Standard RoPE rotates pairs of floats.
             # Our Z is already complex. So 1 complex number = 1 pair.
             # So we need 'dim' rotation factors.
             pass
        
        # Simple broadcast: Rotate each complex element by its frequency
        # Recalculate freqs for full dim to be safe
        # freqs_full = 1.0 / (base ** (arange(0, dim) / dim))
        # For this fix, we just assume freqs_cis has correct shape or broadcast
        
        if cis.shape[0] != z.shape[-1]:
             # Expand to match
             cis = torch.cat([cis, cis], dim=-1)[:z.shape[-1]]
...
class HolographicMemory(nn.Module):
    ...
    def forward(self, z, trace_in, ctrl):
        ...
        # Stability fix: Normalize inputs for FFT
...
class HyperparameterEvolution:
    ...
    def evolve(self, current_loss):
        ...
        if improvement < 0.001:
            # print("   [🧬 Mutation] Stagnation detected. Mutating.")
            choice = random.choice(["lr", "lambda_curiosity", "act_threshold"])
            if choice == "lr":
                CONFIG["lr"] *= random.choice([0.8, 1.2]) # Gentle mutation
...
class CircadianRhythm(nn.Module):
    ...
    def sleep_cycle(self, model):
        ...
        with torch.no_grad():
            ...
            # Dream Replay
            dream_z = torch.randn(1, CONFIG["dim"], dtype=torch.cfloat).to(DEVICE)
            model.cell(dream_z) 
...
class EnhancedUberCRSN(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        # [NEW] True Holographic Memory (FFT)
    ...
    def embed(self, idx):
        ...
        return self.rope(z, step=idx) # Pass step to RoPE
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        if hidden is None:
            ...
            # Holographic trace persistent per batch
...
def train():
    ...
    print(f"--- Training SACRSN v81 (Stabilized) ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            # [FIX] Clamp loss to avoid immediate explosion
...
if __name__ == "__main__":
    FILENAME = "sacrsn_v81_stable.pth"
