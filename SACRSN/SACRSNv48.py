# ============================================================
# SACRSN v45: STABLE HYPER-INTEGRATION (CALIBRATED)
# ------------------------------------------------------------
# 1. CORE: Complex-Valued, Residual Cell, Context-Attention.
# 2. MATH: EBM Graph, Variational KL, Graves ACT, Soft EAFT.
# 3. META: Hierarchical Introspection, Grounding, Meta-Energy.
# 4. FIXES INCLUDED:
#    - All v48 Stability Patches (Optimizer, Dimensions, Types).
#    - [NEW] Calibrated Injection Test (Prevents NaN explosion).
#    - [NEW] Logic Rule Probability Normalization (Shows actual rules).
# ============================================================
...
# [FIXED] Context-Aware Attention (Dimension Fix)
class ContextAwareAttention(nn.Module):
    def __init__(self, dim):
        ...
        # FIX: Project to dim*2 to match flattened complex query
...
# [FIXED] MetaIntrospectionTracker (Real/Complex Fix)
class MetaIntrospectionTracker(nn.Module):
    ...
    def forward(self, z, h_prev, c_prev):
        # FIX: Check if z is complex
        if torch.is_complex(z):
            z_flat = torch.cat([z.real, z.imag], dim=-1)
        else:
            z_flat = z # Already real (from previous level)
...
class HierarchicalMetaTracker(nn.Module):
    ...
    def forward(self, z, h_prev_tuple, c_prev_tuple):
        ...
        # Level 1 (Complex Input)
        h1, c1, kl1, mu1 = self.levels[0](z, h1_prev, c1_prev)
        
        # Level 2 (Real Input from L1)
...
# [FIXED] Enhanced Memory Stack (Cast to complex)
class EnhancedMemoryStack(nn.Module):
    ...
    def forward(self, z, mem, ptr, ctrl):
        ...
        # FIX: Cast flat memory to complex for attention
...
def train():
    ...
    print(f"--- Training SACRSN v48 (Calibrated Finality) ---")
...
def advanced_phase_analysis(model):
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            _, hidden, _, _, _, _, _, _, _, _, _, _ = model(x, hidden)
            zs.append(hidden[0])
...
# [FIXED] Dream Mode (Correct Complex Handling)
...
# [FIXED] Calibrated Injection Test
def injection_test(model):
    print("\n--- Introspection Injection Test (Calibrated) ---")
    model.eval()
...
def extract_logic_rules(model):
    ...
    # Lower threshold to see actual learned transitions
...
if __name__ == "__main__":
    ...
    torch.save(trained_model.state_dict(), "sacrsn_v48_fixed.pth")
    print("Complete.")
