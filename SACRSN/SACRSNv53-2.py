# ============================================================
# SACRSN v53-2: THE OMNI-INTEGRATION (COMPLETE)
# ------------------------------------------------------------
# 1. CORE: v52 Stability (Wirtinger Math, NaN Guards).
# 2. PSYCHE: v51 Ethics, Reframing, & Pacing (RESTORED).
# 3. DIAGNOSTICS: Complete Suite + Dead Code Analysis.
# 4. FIXES: 
#    - Restored Symbol Consistency Loss (v51).
#    - Fixed Memory Stack Slicing.
#    - Fixed Attention EEG Shape.
# ============================================================
...
# ==========================================
# 3. Complex Primitives (v52 Stable)
# ==========================================
...
class HierarchicalMetaTracker(nn.Module):
    def __init__(self, dim, depth=2):
        ...
        for i in range(depth):
            level_dim = dim // (2 ** i); level_dim = max(level_dim, 8)
...
# ==========================================
# 7. Master Model (SacrsnV53Ultimate)
# ==========================================
class SacrsnV53Ultimate(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        metrics = {"halt": 0, "energy": 0, "kl": 0, "ground": 0, "meta_E": 0, 
                   "ethics": 0, "drift": 0, "probe_logit": 0}
        ...
        for t in range(CONFIG["max_depth"]):
            ...
            if training:
                ...
                self.reframer.label_belief(sym_idx) # Track beliefs
...
def train():
    model = SacrsnV53Ultimate(vocab_size, CONFIG["dim"]).to(DEVICE)
    optimizer = AdaptiveOptimizer(model, base_lr=CONFIG["lr"])
    print(f"--- Training SACRSN v53 (Golden Master) ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            L_probe = F.binary_cross_entropy_with_logits(metrics["probe_logit"].mean(), probe_target.mean())
            ...
            loss = CONFIG["lambda_nll"] * nll + \
                   ...
                   CONFIG["lambda_drift"] * metrics["drift"] + \
...
def visualize_all(model):
    ...
    for _ in range(200):
        with torch.no_grad():
            ...
            stack_depths.append(avg_stack.item() if isinstance(avg_stack, torch.Tensor) else 0)
            kl_vals.append(metrics["kl"].item())
            energies.append(metrics["energy"].item())
    ...
    ax1.plot(stack_depths, color='purple', label='Stack Depth')
    ax1.legend()
    ax2.plot(kl_vals, color='green', label='Introspection (KL)')
    ax2.legend()
    ax3.plot(energies, color='orange', label='Energy')
    ax3.legend()
...
def advanced_phase_analysis(model):
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            _, hidden, _, _, _, _, _, _, _, = model(x, hidden, training=False)
            zs.append(hidden[0])
    ...
    plt.title("Advanced Phase Coherence")
    plt.savefig("6_advanced_phase.png")
    plt.close()
    print("Saved 6_advanced_phase.png")
...
def injection_test(model):
    ...
    try:
        ...
        print("Injection caused System Shock (Expected).")
...
def plot_attention_dynamics(model):
    ...
    if len(activations) > 0:
        map_data = np.stack(activations) # Shape (8, 64)
        ...
        plt.savefig("8_attention_eeg.png")
        plt.close()
        print("Saved 8_attention_eeg.png")
...
def plot_symbol_utilization(model):
    ...
    with torch.no_grad():
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            # Correct unpacking: 9 args returned by model
            out = model(x, hidden, training=False)
            hidden = out[1] # Tuple of hidden states
            curr_sym = out[2] # Hard Sym Index (Tensor or None)
    ...
    plt.title(f"Symbol Utilization Spectrum (Active: {active_symbols}/{CONFIG['symbols']})")
    plt.savefig("9_symbol_spectrum.png")
    plt.close()
    print("Saved 9_symbol_spectrum.png")

def plot_ponder_profile(model):
    print("\n--- Profiling Computation Depth ---")
    model.eval()
    x = torch.tensor([[char_to_ix["T"]]], device=DEVICE)
    z = model.embed(x).squeeze(1)
    probs_at_step = []
    for t in range(CONFIG["max_depth"]):
        z_proc, halt_logit, _ = model.cell(z)
        p = torch.sigmoid(halt_logit)
        probs_at_step.append(p.item())
    plt.figure(figsize=(8, 4))
    plt.plot(probs_at_step, marker='o', color='orange')
    plt.title("Halting Probability by Depth")
    plt.ylim(0, 1)
    plt.savefig("10_ponder_profile.png")
    plt.close()
    print("Saved 10_ponder_profile.png")
...
def plot_semantic_drift(model):
    ...
    plt.savefig("11_semantic_drift.png")
    plt.close()
    print("Saved 11_semantic_drift.png")
...
if __name__ == "__main__":
    ...
    generate_text(trained_model, start_str="True", length=200)
    ...
    torch.save(trained_model.state_dict(), "sacrsn_v53_golden_master.pth")
    ...
    try:
        ...
        files.download("sacrsn_v53_golden_master.pth")
        ...
        files.download("8_attention_eeg.png") # Updated
        files.download("9_symbol_spectrum.png")
        files.download("10_ponder_profile.png")
        files.download("11_semantic_drift.png")
