...
CONFIG = {
    ...
    # --- TRAINING ---
    "epochs": 300,           # Long run for perfect convergence
...
def train():
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            if epoch % 20 == 0:  # Adjusted to print every 20 epochs for more frequent feedback
                avg_loss = total_loss / len(data_tensor)
                avg_ponder = total_ponder / len(data_tensor)
                current_lr = scheduler.get_last_lr()[0]
                print(f"Epoch {epoch:04d} | Loss: {avg_loss:.4f} | Ponder: {avg_ponder:.2f} | LR: {current_lr:.6f}")
