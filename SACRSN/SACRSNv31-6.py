# ============================================================
# SACRSN v35 (FINAL FIXED): THE INTEGRATED BI-CAMERAL EDITION
# Fixes: PPX/LR Logging, Topology Word Labeling
# ============================================================
...
CONFIG = {
    ...
    # Dual VQ Settings (v34)
    "n_syntax_symbols": 64,        # Fast Grammar
    "n_semantic_symbols": 128,     # Slow Concepts
    ...
    # Constraints & Topology (v33)
    "graph_bias_scale": 0.7,
...
FULL_TEXT = V33_TEXT * 10 + raw_data 
        ...
        if prev_idx is not None and self.training:
             with torch.no_grad():
                for i in range(len(idx)):
                    ...
                    adj_matrix[r, c] = adj_matrix[r, c] * 0.995 + 1.0
...
class BiCameralCRSN(nn.Module):
    def __init__(self, vocab_size, dim):
        ...
        self.vq = DualScaleVQ(dim, CONFIG["n_syntax_symbols"], CONFIG["n_semantic_symbols"], decay=CONFIG["decay"])
...
def train():
    ...
    print(f"--- Training SACRSN v35 (Bi-Cameral + BPE) ---")
        ...
        for epoch in range(CONFIG["epochs"]):
            ...
            for i, (x_batch, y_batch) in enumerate(loader):
                ...
                for t in range(CONFIG["seq_len"]):
                    ...
                    loss_step = loss_pred + (CONFIG["ponder_penalty"]*ponder) + vq_loss + (CONFIG["ethical_weight"]*eth_loss) + loss_diversity
                    loss_seq += loss_step
                    ponder_seq += ponder
                ...
                if i % 20 == 0:
                     ...
                     print(f"Ep {epoch} | Bt {i:03d} | Loss: {loss_seq.item():.4f} | ACT: {steps:.2f} | PPX: {ppx:.1f} | LR: {lr:.2e}")
...
# ==========================================
# 9. Visualization & Diagnostics (Fixed Labels)
# ==========================================
def visualize_all(model):
    ...
    # 1. Semantic Topology
    print("Generating Semantic Topology Graph...")
    ...
    # INCREASED SCAN LIMIT FOR BETTER LABELS
    ...
    for i in range(CONFIG["n_semantic_symbols"]):
        ...
        if tokens:
            # Simple heuristic: ignore overly common short words for labeling uniqueness
    ...
    nx.draw_networkx_nodes(G, pos, node_color='#a0cbe2', node_size=600, alpha=0.9)
    nx.draw_networkx_labels(G, pos, labels=node_labels, font_size=8, font_weight="bold")
    nx.draw_networkx_edges(G, pos, alpha=0.2, edge_color='gray', arrowstyle='->', arrowsize=10)
    plt.title("1_semantic_topology (NetworkX)")
    plt.savefig("1_semantic_topology.png")
    plt.close()

    # 2. Dream & Diagnostics
    print("Running Inference Scan for MRI/Phase/ACT...")
    ...
    # 5_diagnostic_heatmap (Dual)
    print("Generating 5_diagnostic_heatmap...")
    ...
    plt.yticks(range(3), ['Syntax ID', 'Semantic ID', 'ACT Load'])
