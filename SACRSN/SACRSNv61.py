# ============================================================
# SACRSN v61.1: THE COGNITIVE INTEGRATION (DEBUGGED)
# ------------------------------------------------------------
# 1. BASE: v60 (Stable, EBM, Variational, Full Stack).
# 2. RESTORED: v34 Psychology Logic (Context vs. Meaning Reframing).
# 3. FIXES:
#    - Fixed 'classify_reframe' argument mismatch (loss vs energy).
#    - Fixed 'visualize_all' unpacking error.
# ============================================================
...
CONFIG = {
    ...
    # Psychology Parameters (v34 Logic Restored)
    "reframe_window": 8,
    "entropy_threshold": 0.3, # Low entropy = High Certainty
    "rigidity_limit": 0.9,
    "pacing_interval": 5,     
    
    # Physiology Parameters
    "enable_homeostasis": False, # God Mode
...
# --- v53/v33 PSYCHOLOGY (Integrated) ---
class MetaBeliefTracker:
    def __init__(self, n_symbols):
        # [RESTORED] Full stats from v33
        self.stats = {
            s: {
                "count": 0, 
                "avg_energy": 0.0, # Mapped from loss
                "avg_ponder": 0.0,
                "rigidity": 0.0
    ...
    def update(self, sym, energy, ponder, changed):
        ...
        # Rigidity: increases if symbol stays same, decreases if changed
...
class EthicalGate:
    ...
    def allow(self, meta):
        # Don't change beliefs that are too rigid (Core Values)
...
class BeliefReframer(nn.Module):
    ...
    # [RESTORED] v33 Decision Logic
    def classify_reframe(self, energy, ponder):
        entropy = self.belief_entropy()
        if entropy is None: return None
        
        # Condition A: Rigid + Overthinking = Meaning Crisis (Change Symbol)
        if entropy < self.entropy_threshold and ponder > 0.8: # Ponder is 0-1 sigmoid
            return "meaning"
        
        # Condition B: Rigid + High Energy (Error) = Context Crisis (Change Weights)
        if entropy < self.entropy_threshold and energy > 0.0: # EBM energy > 0 is high
            return "context"
            
        return None

    # [ADAPTED] Meaning Reframe for EBM
    ...
    # [ADAPTED] Context Reframe for EBM
...
def train():
    ...
    print(f"--- Training SACRSN v61.1 (Cognitive Integration + Debugs) ---")
...
if __name__ == "__main__":
    ...
    torch.save(trained_model.state_dict(), "sacrsn_v61_cognitive.pth")
