# ============================================================
# SACRSN v43: THE HYPER-INTEGRATED SYSTEM (FIXED)
# ------------------------------------------------------------
# Includes fixes for:
# 1. Real/Complex mismatch in Hierarchical Introspection
# 2. Dimension mismatch in Context-Aware Attention
# 3. Flat/Complex casting in Enhanced Memory
# ============================================================
...
# [FIXED] Context-Aware Attention (Dimension Mismatch Fix)
class ContextAwareAttention(nn.Module):
    def __init__(self, dim):
        ...
        # FIX: Project context to dim*2 (128) to match flattened complex query
...
# [FIXED] Complex Attention (Standard)
...
# ==========================================
# 4. Advanced Modules (v41 Meta + v43 Upgrades)
# ==========================================

# [FIXED] MetaIntrospectionTracker (Handles Real/Complex Inputs)
class MetaIntrospectionTracker(nn.Module):
    ...
    def forward(self, z, h_prev, c_prev):
        # FIX: Check if z is complex before accessing .real/.imag
        if torch.is_complex(z):
            z_flat = torch.cat([z.real, z.imag], dim=-1)
        else:
            # If z is real (from a previous hierarchy level), use it directly
            z_flat = z
...
class HierarchicalMetaTracker(nn.Module):
    ...
    def forward(self, z, h_prev_tuple, c_prev_tuple):
        ...
        # Level 1 (Input is Complex z)
        h1, c1, kl1, mu1 = self.levels[0](z, h1_prev, c1_prev)
        
        # Level 2 (Input is Real h1 from Level 1)
        # The corrected MetaIntrospectionTracker handles this Real input now
...
# [FIXED] Enhanced Memory Stack (Cast to complex for attention)
class EnhancedMemoryStack(nn.Module):
    ...
    def forward(self, z, mem, ptr, ctrl):
        ...
        # FIX: Create complex view of memory for attention
...
class EnhancedUberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, injected_thought=None, 
        ...
        halt_cost, energy_cost, kl_cost, ground_cost, meta_cost = 0, 0, 0, 0, 0
        mem_efficiency_log = []
        ...
        return logits, next_hidden, halt_cost, energy_cost, kl_cost, ground_cost, meta_cost, (avg_mu_l1, avg_mu_l2), stack_depth_log, aux_class, aux_recon, avg_mem_efficiency
...
class AdaptiveOptimizer:
    ...
    def step(self, loss, kl_value):
        ...
        if len(self.kl_history) > 5:
            kl_change = abs(self.kl_history[-1] - self.kl_history[-5])
            if kl_change < 0.01: lr = self.base_lr * CONFIG["adaptive_lr_factor"]
            else: lr = self.base_lr / CONFIG["adaptive_lr_factor"]
        else:
            lr = self.base_lr
        ...
        self.opt.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.opt.param_groups[0]['params'], CONFIG["grad_clip"])
        self.opt.step()
        return lr
...
def train():
    ...
    print(f"--- Training SACRSN v43 (Hyper-Integrated Fixed) ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            logits, hidden, halt, energy, kl, ground, meta, mus, _, aux_c, aux_r, _ = model(x, hidden)
...
def visualize_all(model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(data_tensor) - 1):
            x = data_tensor[i].view(1, 1)
            _, hidden, _, energy, kl, _, _, _, depth, _, _, eff = model(x, hidden)
    ...
    plt.suptitle("Hyper-Cognitive MRI")
...
def dream_mode(model):
    ...
    for _ in range(50):
        ...
        z = model.vq.codebook[next_idx].unsqueeze(0)
        q_logits = model.quantum_layer(z.to(DEVICE))
...
def injection_test(model):
    print("\n--- Introspection Injection Test ---")
    model.eval()
    x = torch.tensor([[char_to_ix["T"]]], device=DEVICE)
    _, _, _, _, kl_base, _, _, _, _, _, _, _ = model(x, None)
    inject = torch.complex(torch.randn(1, 64).to(DEVICE)*5, torch.randn(1, 64).to(DEVICE)*5)
    _, _, _, _, kl_inj, _, _, _, _, _, _, _ = model(x, None, injected_thought=inject)
    print(f"Normal KL: {kl_base.item():.4f} | Injected KL: {kl_inj.item():.4f}")
    if kl_inj > kl_base * 1.5: print("SUCCESS: Injection Detected.")
    else: print("FAIL: Injection Ignored.")
...
if __name__ == "__main__":
    ...
    torch.save(trained_model.state_dict(), "sacrsn_v43_final.pth")
    print("Complete.")
