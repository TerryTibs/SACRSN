# ============================================================
# SACRSN v82: THE STABLE MONOLITH (NaN-Fixed)
# ------------------------------------------------------------
...
# 4. FIXES: Numerical Clamping on VQ and KL (Prevents NaN).
# 5. FIXES: ZeroDivisionError guard in training loop.
# ============================================================
...
print(f"Running SACRSN v82 on: {DEVICE}")
...
CONFIG = {
    ...
    "lambda_halt": 0.01,
    "lambda_energy": 0.1,
    "lambda_kl": 0.01,          # Reduced to prevent early explosion
...
class ComplexRoPE(nn.Module):
    ...
    def forward(self, z, step=0):
        # Broadcast freqs to match z dimension
        # z: [Batch, Dim]
        safe_step = step % self.freqs_cis.shape[0]
        cis = self.freqs_cis[safe_step] # [Dim/2]
        
        # Manually expand to [Dim] by repeating
        # Complex numbers in PyTorch are scalars, so we need 1 rot per scalar
...
class HolographicMemory(nn.Module):
    ...
    def forward(self, z, trace_in, ctrl):
        ...
        # Stability: Normalize to prevent explosion
...
class GroundedMetaGraphVQ(nn.Module):
    ...
    def forward(self, z, prev_symbol_dist=None, prev_entropy=None, temp=1.0, manual_idx=None):
        ...
        # [FIX] Safer Distance Calculation
        # d = |z|^2 + |e|^2 - 2ze
        # Clamp to avoid negative sqrt issues if rounding errors occur
...
def train():
    ...
    print(f"--- Training SACRSN v82 (Stabilized) ---")
    ...
    for epoch in range(CONFIG["epochs"]):
        ...
        for i in range(len(data_tensor) - 1):
            ...
            # [FIX] Clamp loss to prevent immediate explosion
...
if __name__ == "__main__":
    FILENAME = "sacrsn_v82_stable.pth"
