# ============================================================
# SACRSN v32: THE STRICT BATCHED EDITION
# Changes: Batch Size increased. Training Loop adapted. 
...
CONFIG = {
    ...
    # Training
    "epochs": 3000,
    "learning_rate": 1e-3, # Kept same as original
    "grad_clip": 0.5,
    "eps": 1e-6,
    "warmup_epochs": 0
}
...
# [CHANGE] Repeat text so it's long enough to split into 32 columns
# We need at least enough tokens to fill [Batch_Size, Min_Seq_Len]
if len(TEXT_DATA) < CONFIG["batch_size"] * 100:
...
# [CHANGE] 1. The Evaluation Tensor (1D) - For Analysis tools that expect 1 stream
eval_tensor = torch.tensor([char_to_ix[c] for c in TEXT_DATA], dtype=torch.long).to(DEVICE)

# [CHANGE] 2. The Training Tensor (2D) - For Fast Training
n_batches = len(eval_tensor) // CONFIG["batch_size"]
train_tensor = eval_tensor[:n_batches * CONFIG["batch_size"]]
train_tensor = train_tensor.view(CONFIG["batch_size"], -1) # Shape: [32, Sequence_Length]
...
class GraphMemoryVQ(nn.Module):
    ...
    def forward(self, z, prev_symbol_idx=None):
        ...
        if prev_symbol_idx is not None:
            # PyTorch advanced indexing handles batches automatically here
...
class EthicalConstraint(nn.Module):
    ...
    def forward(self, prev_sym, curr_sym, adjacency):
        ...
        # PyTorch CrossEntropy handles Batches automatically (Mean reduction is default)
...
class UberCRSN(nn.Module):
    ...
    def forward(self, input_ids, hidden=None, prev_sym=None):
        ...
        for t in range(CONFIG["max_recursion_depth"]):
            ...
            remain = remain - p
            ponder_cost += still_running.mean() # Model ALREADY has mean, no change needed
...
# ==========================================
# 7. Training Engine (MODIFIED FOR BATCHING)
# ==========================================
def train():
    ...
    print(f"--- Training SACRSN v32 (Batched) ---")
    
    # [CHANGE] Iterate over Sequence Length (Columns), not total data
    ...
    try:
        for epoch in range(CONFIG["epochs"]):
            ...
            for i in range(steps_per_epoch - 1):
                # [CHANGE] Slice the batch columns
                ...
                # [CHANGE] Standard Mean Reduction for batching (implied by default, but safe to be explicit)
                loss_pred = F.cross_entropy(logits, y) 
                
                # 'ponder' is already a mean scalar from the model class
                loss_ponder = CONFIG["ponder_penalty"] * ponder
                
                # [CHANGE] Entropy Calculation: Normalize across batch properly
                probs = F.softmax(logits, dim=-1)
                # Sum over vocab (dim -1), then Mean over batch
                loss_entropy = -entropy_weight * (-(probs * torch.log(probs + 1e-8)).sum(dim=-1).mean())
                
                # [CHANGE] Handle batch dimension for history buffer
                curr_onehot = F.one_hot(sym_idx, CONFIG["n_symbols"]).float()
                with torch.no_grad():
                    # Collapse batch dimension using mean
                ...
                # vq_loss and eth_loss are already Scalars from the model class
            ...
            if epoch % 50 == 0:
                # [CHANGE] Correct logging divisor
...
# ==========================================
# 8. Visualization Suite (UNTOUCHED LOGIC)
# ==========================================
def visualize_all(model):
    ...
    with torch.no_grad():
        # [CHANGE] Use 'eval_tensor' (1D) to match original visualization logic
        for i in range(len(eval_tensor) - 1):
            x = eval_tensor[i].view(1, 1) # Force Batch Size 1
...
def extract_logic_rules(model, dataset):
    ...
    with torch.no_grad():
        # [CHANGE] Use the passed dataset (which will be eval_tensor 1D)
...
# ==========================================
# 10. Advanced Interaction (UNTOUCHED)
# ==========================================
...
if __name__ == "__main__":
    ...
    # 2. Analysis using 1D Eval Tensor (Passed explicitly to avoid crash)
